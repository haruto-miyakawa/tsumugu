      ...
      <Card noPad>
        {articles.length === 0 ? (
          <div className="flex flex-col items-center gap-4 py-16 px-6 text-center">
            ...
            <p className="text-sm text-mute">まだ記事がありません。</p>
          ...
          <ul className="divide-y divide-rule">
            {articles.map((a) => (
              <li key={a.id}>
                ...
                >
                  <div className="w-[3px] self-stretch rounded-full bg-accent opacity-60" />
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-sans font-medium text-ink truncate group-hover:text-accent transition-colors">
                      {a.title}
                    </p>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className={`text-[10px] px-1.5 py-0.5 font-mono rounded-sm ${STATUS_CLASS[a.status]}`}>
                        {STATUS_LABEL[a.status]}
                      </span>
                      <span className="text-[11px] font-mono text-mute">{a.wordCount.toLocaleString()}字</span>
                  ...
                  <div className="shrink-0 text-right">
                    <p className="font-display text-lg text-accent leading-none">{a.progress}%</p>
