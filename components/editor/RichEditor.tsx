...
export function RichEditor({ initialHtml, isMobile, kicker, date, tags, onUpdate }: Props) {
      ...
      <div className="flex-1 overflow-y-auto">
        <article
          className={`mx-auto py-10 ${
            isMobile ? "px-5 max-w-full" : "px-10 max-w-[640px]"
          }`}
        >
          {kicker && (
            <p className="font-mono text-[10px] tracking-widest text-mute uppercase mb-5">
              {kicker}
            </p>
          )}
          {date && (
          ...
          {tags && tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-10 pt-6 border-t border-rule">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="text-[11px] font-mono text-mute bg-surface border border-rule px-2 py-0.5 rounded-sm"
                >
                  {tag}
                </span>
              ))}
